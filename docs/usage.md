...
```bash
nextflow run nf-core/magmap --input samplesheet.csv --genome GRCh37 -profile docker
```
...
First, go to the [nf-core/magmap releases page](https://github.com/nf-core/magmap/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
...
* `docker`
  ...
  * Pulls software from Docker Hub: [`nfcore/magmap`](https://hub.docker.com/r/nfcore/magmap/)
* `singularity`
